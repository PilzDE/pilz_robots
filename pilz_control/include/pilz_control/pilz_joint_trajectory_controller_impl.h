...
/**
 * @brief Calculate absolute acceleration based on two velocities (`current_velocity` and `old_velocity`)
    ...
    const ros::Duration& period) const
{
  for (unsigned int i = 0; i < JointTrajectoryController::getNumberOfJoints(); ++i)
  {
    if (acceleration_joint_limits_.at(i))
    {
      ...
      const double& acceleration = calculateAbsoluteAcceleration(new_velocity, old_velocity, period);
