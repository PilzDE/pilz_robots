...
/**
 * @brief Tests that an asynchronous call to waitForPublisher waits until someone publishes on the topic.
 */
TEST(WaitForPublisherTests, testAsyncCall)
{
  using namespace pilz_utils;
  std::string topic_name{"/test_topic"};

  auto wait_future = std::async([topic_name](){ waitForPublisher<std_msgs::Empty>(topic_name); return true; });
...
{
  ros::init(argc, argv, "unittest_wait_for_publisher");
  ros::NodeHandle nh;
