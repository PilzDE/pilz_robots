...
TEST(SleepTest, testSleepSimTime)
{
  ...
  EXPECT_NEAR(ros::Time::now().toSec(), (starting_time + sleep_time).toSec(), TIME_COMPARISON_TOLERANCE_S)
      << "Simulation time did not progress as expected during sleep.";
}
...
TEST(SleepTest, testSleepInvalidSimTime)
{
  ...
  EXPECT_NEAR(ros::Time::now().toSec(), starting_time.toSec(), TIME_COMPARISON_TOLERANCE_S)
      << "Simulation time did progress unexpectedly during sleep.";
}
