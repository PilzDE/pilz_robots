...
void Stop1Executor::workerThreadFun()
{
  ...
  while (!terminate_)
  {
    ...
    task.execute();    // | the state machine since new sto updates need to be
                       // handled
    // | during service calls.
    sm_lock.lock();  // |

    task.signalCompletion();  // Could add Task to Queue and does process_event
                              // on the state machine. Needs lock.
  }
}
