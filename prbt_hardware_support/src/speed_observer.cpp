...
SpeedObserver::SpeedObserver(ros::NodeHandle &nh, std::string &reference_frame,
    ...
    : nh_(nh), reference_frame_(reference_frame),
      frames_to_observe_(frames_to_observe) {
  frame_speeds_pub_ =
      nh.advertise<FrameSpeeds>(FRAME_SPEEDS_TOPIC_NAME, DEFAULT_QUEUE_SIZE);
  sto_client_ =
      nh.serviceClient<std_srvs::SetBool>(STO_SERVICE, DEFAULT_QUEUE_SIZE);
}
...
tf2::Vector3 SpeedObserver::getPose(const std::string &frame,
                                    const ros::Time &now) const {
  tf2::Vector3 v;
  geometry_msgs::TransformStamped transform{
      tf_buffer_.lookupTransform(reference_frame_, frame, now)};
...
void SpeedObserver::startObserving(double frequency) {
  ...
  while (ros::ok() && !terminate_) {
    ...
    for (const auto &frame : frames_to_observe_) {
      ...
      double curr_speed{speedFromTwoPoses(previous_poses.at(frame), curr_pos,
                                          (now - previous_t).toSec())};
      if (!isWithinLimits(curr_speed)) {
        ROS_ERROR("Speed %.2f m/s of frame >%s< exceeds limit of %.2f m/s",
                  curr_speed, frame.c_str(), SPEED_LIMIT);
        triggerStop1();
      }
...
FrameSpeeds SpeedObserver::createFrameSpeedsMessage(
...
void SpeedObserver::triggerStop1() {
  ...
  if (!call_success) {
    ROS_ERROR_STREAM(
        "No success calling service: " << sto_client_.getService());
  }
