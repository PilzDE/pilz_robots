    ...
    : nh_(nh)
{
  brake_test_srv_ = nh_.advertiseService(nh_.getNamespace() + TRIGGER_BRAKETEST_SERVICE_NAME,
                                         &CANOpenBrakeTestAdapter::triggerBrakeTests,
                                         this);
...
ros::Duration CANOpenBrakeTestAdapter::getBrakeTestDuration(const std::string &node_name)
{
  ...
  {
    throw CANOpenBrakeTestAdapterException(srv.response.message,
                                           BrakeTestErrorCodes::GET_DURATION_FAILURE);
  }
...
CANOpenBrakeTestAdapter::BrakeTestStatus CANOpenBrakeTestAdapter::getBrakeTestStatusForNode(const std::string &node_name)
{
  canopen_chain_node::GetObject srv;
  srv.request.node = node_name;
...
std::vector<std::string> CANOpenBrakeTestAdapter::getNodeNames()
{
  ...
  {
    throw CANOpenBrakeTestAdapterException("Could not read node names",
  ...
  for (auto& rpci : rpc)
  {
    ...
    catch(const XmlRpc::XmlRpcException& e)
    {
      ...
      throw CANOpenBrakeTestAdapterException("Could not read parameter " + NODE_BRAKETEST_ENABLED_PARAMETER + " of " + node_name,
                                        BrakeTestErrorCodes::GET_NODE_NAMES_FAILURE);
    }
