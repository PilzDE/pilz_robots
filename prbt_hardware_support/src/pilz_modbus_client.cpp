...
{
PilzModbusClient::PilzModbusClient(
    ros::NodeHandle& nh, const std::vector<unsigned short>& registers_to_read,
  ...
  , modbus_read_pub_(nh.advertise<ModbusMsgInStamped>(
        modbus_read_topic_name, DEFAULT_QUEUE_SIZE_MODBUS))
  , modbus_write_service_(
        nh.advertiseService(modbus_write_service_name,
                            &PilzModbusClient::modbus_write_service_cb, this))
{
}
                            ...
                            unsigned int retries, const ros::Duration& timeout)
{
  for (size_t retry_n = 0; retry_n < retries; ++retry_n)
  {
    ...
    ROS_ERROR_STREAM("Connection to " << ip << ":" << port << " failed. Try("
                                      << retry_n + 1 << "/" << retries << ")");
...
bool PilzModbusClient::init(const char* ip, unsigned int port)
{
  ...
  {
    ROS_ERROR_STREAM("Modbus-client not in correct state: "
                     << state_ << "expected:" << State::initializing);
...
void PilzModbusClient::run()
{
  ...
  while (ros::ok() && !stop_run_.load())
  {
    ...
    std::vector<std::vector<unsigned short>> blocks =
        splitIntoBlocks(registers_to_read_);

    unsigned short index_of_first_register =
        *std::min_element(registers_to_read_.begin(), registers_to_read_.end());
    int num_registers = *std::max_element(registers_to_read_.begin(),
                                          registers_to_read_.end()) -
                        index_of_first_register + 1;
      ...
      for (auto& block : blocks)
      {
        ...
        if (write_reg_bock)
        {
          block_holding_register = modbus_client_->writeReadHoldingRegister(
              static_cast<int>(write_reg_bock->start_idx),
        ...
        {
          block_holding_register = modbus_client_->readHoldingRegister(
              static_cast<int>(index_of_first_register_block),
              static_cast<int>(num_registers_block));
        }
        for (uint i = 0; i < num_registers_block; i++)
          holding_register[i + index_of_first_register_block -
                           index_of_first_register] = block_holding_register[i];
      }
    }
    ...
    ModbusMsgInStampedPtr msg{ ModbusMsgInBuilder::createDefaultModbusMsgIn(
        index_of_first_register, holding_register) };
...
PilzModbusClient::splitIntoBlocks(std::vector<unsigned short>& in)
{
  std::vector<std::vector<unsigned short>> out;
  std::sort(in.begin(),
            in.end());  // sort just in case to be more user-friendly
