...
static const std::string SPEED_MONITOR_SERVICE_NAME
  {"/prbt/manipulator_joint_trajectory_controller/monitor_cartesian_speed"};
...
TEST_F(DisableSpeedMonitoringTest, testDisableSpeedMonitoring)
{
  ...
  ON_CALL(service_mock, callback(_, _))
    .WillByDefault(DoAll(SetArgReferee<1>(response), Return(true)));
