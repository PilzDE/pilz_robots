 ...
 * AsyncTest::triggerClearEvent. During the test setup gates between the steps
 * with one or more clear events. Allow passing on by calling triggerClearEvent
 * after a test.
 *
 * \e Usage:<br>
 ...
 * completion, else it would fail. This can be achieved via:
 *
 * \code
...
public:
   ...
   * it will unblock as soon as all clear events are triggered. Else the event
   * is put on the waitlist. This waitlist is emptied upon a call to barricade.
   *
  ...
  /**
   * @brief Will block until the event given by clear_event is triggered.
  ...
  /**
   * @brief Will block until all events given by clear_events are triggered.
